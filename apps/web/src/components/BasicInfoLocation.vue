...
<template>
  <div :grid="`~ rows-[repeat(3,auto)] gap-x-6 ${location.photo ? 'cols-[auto_1fr]' : 'cols-[auto_1fr_auto]'}`"
    :data-inverted="location.isDark && location.isAtm ? true : undefined">
    ...
    <LocationExternalUrl v-if="!location.photo && location.url && progress > 0.5" :location self-start mr-20 my--8 />
    ...
    <p v-if="location.address" leading-18 :grid="`~ cols-1 row-start-3 ${location.photo ? 'col-span-2' : 'col-span-3'}`"
      text="12 neutral-800 inverted:neutral-800">
      {{ location.address }}
    </p>
  </div>
</template>
