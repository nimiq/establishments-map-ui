      ...
      <Transition name="slide-left">
        <DropdownMenuContent bg-gradient-neutral rounded-6 p-4 shadow absolute top--24 right--10 min-w-152 flex="~ col"
          ...
          <DropdownMenuItem v-if="shareIsSupported" flex="~ items-center" px-14 py-8 bg="hocus:neutral-0/10" rounded-2
            text="neutral-0 hover:text-neutral-100" cursor-pointer @click="share" animate-fade-in animate-duration-100 animate-delay-100 animate-both>
          ...
          <DropdownMenuItem v-if="copyIsSupported" @select.prevent="() => copy()" px-14 py-8
            ...
            transition-colors rounded-2 text="neutral-0 hover:text-neutral-100" animate-fade-in animate-duration-100 animate-delay-150 animate-both>
          ...
          <ReportLocation :location>
            <template #trigger>
              <DropdownMenuItem as="button" flex="~ items-center" px-14 py-8 text-red-1100 cursor-pointer w-full
                bg="hocus:red/20" rounded-2 animate-fade-in animate-duration-100 animate-delay-200 animate-both @select.prevent>
