  ...
  >
    <div class="grid grid-flow-dense grid-cols-[auto_1fr_auto] grid-rows-2 items-center gap-x-2">
      ...
      <button
        ...
        close-btn relative top--4 col-start-3 ml-auto size-24 rounded-full p-8 text-neutral-100 transition @click="$emit('close')"
      />
    </div>
    ...
    <button nq-pill-sm mt-2 text-14 nq-pill-blue>
      {{ cryptocity.url }}
    </button>
  </div>
</template>
