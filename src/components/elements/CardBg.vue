...
<template>
  <template v-if="location.isShop">
    ...
    >
      ...
      >
        <component :is="location.providerLogo" v-if="progress === 1" class="w-6 h-6 zoom-in " />
      </div>
    </div>
  </template>

  <template v-if="location.isAtm && location.providerLogo">
    <div class="absolute bottom-0 right-0 z-20 grid pointer-events-none place-content-center">
      <div
        ...
        data-rings
      />
      <component :is="location.providerLogo" class="w-9 h-9 absolute right-[30px] bottom-[30px]" />
    </div>
  </template>
</template>
